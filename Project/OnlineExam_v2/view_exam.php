...
$(document).ready(function(){
	...
	function update_submit_time(user_id)
	{
		$.ajax({
			url:"user_ajax_action.php",
			method:"POST",
			data:{exam_id:exam_id, user_id:user_id, page:'view_exam', action:'submit_time'},
			success:function(data)
			{
				$('#single_question_area').html(data);
			}
		...
		<div class="card-body">
			<div class="table-responsive">
				<table class="table table-bordered table-hover">
					...
					$exam->query = "
					SELECT user_exam_enroll_table.submit_exam_datetime 
					FROM user_exam_question_answer INNER JOIN user_exam_enroll_table
					ON user_exam_question_answer.exam_id=user_exam_enroll_table.exam_id
					WHERE user_exam_question_answer.user_id = '".$_SESSION['user_id']."' 
					AND user_exam_question_answer.exam_id = '".$exam_id."'
					";
					...
					<tr>
						<td colspan="8" align="right">Submitted Time</td>
						<td align="right"><?php echo $row["submit_exam_datetime"]; ?></td>
					</tr>
					<?php	
